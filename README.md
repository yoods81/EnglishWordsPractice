...
- **Add Word** (Admin only — see above) — add your own words, in one of two modes:
  ...
  - **Meanings and levels are tracked separately for each language track.** Whichever track you're on when a word is looked up automatically (bulk-add, photo import) or manually typed (single word), the app also looks up (or, for a manually typed meaning, fetches in the background) the *other* language's meaning and estimates the *other* language's level for it — using dictionaryapi.dev for English and the free MyMemory API for Korean. So a word added on the English track still shows an English meaning and an English-track level (Year 4/5/6) if you switch to the Korean track, and vice versa. If a word has no formal English dictionary entry (or MyMemory can't translate it directly) but a meaning was found on the other side, that meaning is translated into the missing language instead of leaving it blank. Lookups run a few at a time with an automatic retry, so a word occasionally failing doesn't sink the whole batch.
  - Any word still missing a meaning *for the language track you're currently viewing* shows up under "My added words" with a "⚠️ N words still need a meaning" banner: retry that one word from its own **🔄 Retry** button, retry all of them at once with **Retry All**, or bulk-remove whichever still fail with **Delete All Failed**. The **⚠️ Missing meaning first** button in the "My added words" title bar sorts those words to the top of the list so they're easy to find. Tick the checkbox next to any word(s) and use **🗑️ Delete Selected** (also in the title bar) to remove several at once. You can also edit or delete any word you've added individually.
