...
- **Typing Game** — a falling-words typing game built from the same word list as Spelling practice, with a playful cloud/star backdrop and a short looping background melody (with a 🔊/🔇 mute button tucked in the corner of the play area, remembered between visits). Words spawn at the top of the play area and drift downward — very slowly at first, paced for a beginner who's just learning to type — and typing locks onto whichever falling word your keystrokes match (the one closest to the bottom wins if two share a prefix, so there's never an ambiguous match). Pressing Enter checks a guess explicitly: a wrong entry clears the input and shows a "no matching word" message so you can try again, while a still-valid partial guess is left alone. Finishing a word scores points and clears it; letting one reach the bottom costs one of 5 lives, and the game speeds up — faster falling, more frequent spawns — every 10 words typed correctly, not by score, so spelling out a long word never triggers an early speed-up. Losing all 5 lives ends the round with a final score and, if it beats your best for this level and language, a new high score (kept separately per level, since Year 4 and Year 6 aren't really comparable). Switching to another tab mid-round pauses it in place rather than ending it; coming back resumes exactly where you left off. A level without enough words to play disables **▶ Start Game** with a note to add more first.
