...
- **Word list** — the words for the current level with a per-word mastery percentage. Tap a word or its example sentence to hear it. Typing in the search box looks through *every* level, not just the one you're on, and tags each hit from another level with that level's name — an automatically added word is filed under whichever level was guessed for it, which often isn't the level you happen to be browsing.
